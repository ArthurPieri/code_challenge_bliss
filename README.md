...
3. Para executar os tests:
    ...
    uv run pytest
    ```
    > Note, o teste test_extract_encrypted pode falhar, porém basta executar o pytest novamente
4. Acessar a pasta src:
```bash
cd src/
```
5. Para executar o desafio_etl
```bash
    uv run python desafio_etl.py 
```
6. Para executar o desafio de governança:
```bash
    uv run python governance.py 
```
7. Para executar o airflow localmente você deve acessar a pasta src/orchestration/infra:
```bash
